import { AI_CLOUD_JOB_KINDS } from "@vixmotion/core";
 ...
 * - "replace": the result IS the clip's transformed media â†’ replace the clip's
 *   source media in place (no new library tile).
 * - "newAsset": the result is genuinely new content (generated music, audio
 *   stems, thumbnails) â†’ add it to the media library as a new item.
 */
...
export interface AiKindDef {
   ...
   * stems). Without this, only the single primaryArtifact is imported â€” which
