...
export const Timeline: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // playhead lands back near the left with the rest as lookahead â€” instead of
  ...
  return (
    ...
    >
      {/* â”€â”€ Timeline toolbar (mock: 48px line-icon tools + emerald zoom slider) â”€â”€ */}
      <div className="flex items-center h-12 px-4 gap-4 bg-bg-1 border-b border-border shrink-0 relative z-50">
        <TLTool onClick={undo} disabled={!canUndo()} title="Undo (âŒ˜Z)">
          <Undo2 size={16} aria-hidden />
        </TLTool>
        <TLTool onClick={redo} disabled={!canRedo()} title="Redo (â‡§âŒ˜Z)">
              ...
              {pendingTrackDelete ? (
                ...
                >
                  <p className="text-[11px] font-semibold text-danger">
                    Delete â€œ{pendingTrackDelete.name}â€?
                  </p>
