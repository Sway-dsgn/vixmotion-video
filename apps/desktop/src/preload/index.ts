...
contextBridge.exposeInMainWorld("VixMotion", {
  platform: "desktop",
  ...
  onMenuAction: (cb: (id: string) => void) => {
    ...
    ipcRenderer.on("vixmotion:menu:action", handler);
    return () => ipcRenderer.removeListener("vixmotion:menu:action", handler);
  },
  fs: {
  ...
  export: {
    start: (args: unknown) =>
      new Promise((resolve) => {
        ipcRenderer.once("vixmotion:export-port", (event, meta) => {
          ...
          window.postMessage({ __VixMotionExportPort: true, jobId }, "*", [port]);
          resolve({ jobId });
        });
