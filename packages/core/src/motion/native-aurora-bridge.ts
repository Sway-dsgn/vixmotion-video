...
function getBridge(): NativeAuroraBridge | undefined {
  const scope = globalThis as { VixMotion?: Partial<NativeAuroraBridge> };
  const VixMotion = scope.VixMotion;
  if (VixMotion?.platform === "desktop" && VixMotion.aurora) {
    return VixMotion as NativeAuroraBridge;
  }
  return undefined;
}
