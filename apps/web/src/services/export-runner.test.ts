...
import type { Project, VideoExportSettings, ExportResult } from "@vixmotion/core";
import {
  useExportRunner,
  mimeForExt,
  extForFormat,
...
describe("export-runner pure helpers", () => {
  ...
  it("keeps legal filename characters intact", () => {
    ...
    expect(sanitizeFilenameBase("cafÃ© dÃ©jÃ -vu")).toBe("cafÃ© dÃ©jÃ -vu");
  });
...
describe("useExportRunner showSavePicker fallback", () => {
  ...
  afterEach(() => {
    ...
    delete win.VixMotion;
    delete win.__VixMotionExportPath;
  });
  ...
  it("createDownloadWritable resolves the native ffmpeg output path on desktop", async () => {
    ...
    expect(win.__VixMotionExportPath).toBe("/Users/me/Movies/Rubik.mp4");
  ...
  it("createDownloadWritable rethrows AbortError when the native save dialog is cancelled", async () => {
    ...
    expect(win.__VixMotionExportPath).toBeUndefined();
  });
});
