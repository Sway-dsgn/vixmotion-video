...
import type { MotionComposition } from "@vixmotion/core/motion/types";
...
const SceneCard: React.FC<{ scene: CreationSceneReview; active: boolean }> = ({
  ...
  >
    ...
    <div className="mt-1 text-xs text-white/50">
      {scene.objectCount} object(s) Â· {scene.cameraCount} camera(s) Â· {scene.animationCount}{" "}
      animation(s)
    </div>
      ...
      {!review.available ? (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="mb-3 text-xs text-white/50">
            {review.assetCount} asset(s) Â· {review.sceneCount} scene(s)
          </div>
