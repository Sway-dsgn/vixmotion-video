...
import { ToolcraftNumberInputControl as NumberInput } from "@vixmotion/ui";
import { ToolcraftText as Text } from "@vixmotion/ui";
import { PropertySlider } from "./shell/PropertySlider";
import { useEngineStore } from "../../../stores/engine-store";
import { useProjectStore } from "../../../stores/project-store";
import type { BezierPath, Mask, MaskShape } from "@vixmotion/core";
import { boundsPathFromTransform } from "@vixmotion/core";
...
export const MaskSection: React.FC<MaskSectionProps> = ({ clipId }) => {
  ...
  const [maskEngine, setMaskEngine] =
    useState<import("@vixmotion/core").MaskEngine | null>(null);
  ...
  // surprised by silent filtering â€” they can pick whatever they want.
  const matteSourceOptions = useMemo(() => {
    ...
    try {
      for (const t of getAllTextClips()) {
        opts.push({
          id: t.id,
          label: `Text â€¢ "${t.text.slice(0, 20)}${t.text.length > 20 ? "â€¦" : ""}"`,
        });
      }
    } catch {
  ...
  // transform. We re-derive the path whenever the project changes â€”
  ...
  useEffect(() => {
    ...
    for (const mask of trackMattes) {
      ...
      // Cheap stringify-equality check â€” paths are tiny.
    ...
    if (didChange) {
      // Don't tick the project modifiedAt here â€” this is a derived
