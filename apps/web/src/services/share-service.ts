import { VixMotion_CLOUD_URL } from "../config/api-endpoints";
...
export async function uploadForSharing(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", `${VixMotion_CLOUD_URL}/shares`);
    xhr.send(formData);
  });
}
...
export async function getShareInfo(shareId: string): Promise<ShareInfo | null> {
  try {
    const response = await fetch(`${VixMotion_CLOUD_URL}/shares/${shareId}`);
...
export function getShareDownloadUrl(shareId: string): string {
  return `${VixMotion_CLOUD_URL}/shares/${shareId}/download`;
}
...
export async function checkShareHealth(): Promise<boolean> {
  try {
    const response = await fetch(`${VixMotion_CLOUD_URL}/health`);
    return response.ok;
  } catch {
    return false;
  }
