...
import { getMotionShaderFillDefs } from "@vixmotion/core";
...
describe("ShaderPreviewBrowser", () => {
  ...
  it("filters previews by shader collection while retaining visual selection", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Paper Â· 2/i }));
    expect(screen.getByRole("button", { name: /Paper Â· 2/i })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(
