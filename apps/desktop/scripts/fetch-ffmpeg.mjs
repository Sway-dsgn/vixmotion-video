...
//   - darwin-arm64: OSXExperts (osxexperts.net) â€” ffmpeg-static's arm64 is
...
function extractFromZip(zipBuffer, member, slot) {
  ...
  // if it is missing.
  const tmpZip = path.join(os.tmpdir(), `VixMotion-ffmpeg-${slot}-${process.pid}.zip`);
  try {
...
async function writeManifest(slots) {
  ...
  const manifest = {
    ...
    binaries,
    note: "Binaries are gitignored. Run `pnpm --filter @vixmotion/desktop fetch:ffmpeg` (host slots) or `node scripts/fetch-ffmpeg.mjs --all`. Sources/versions differ per slot (see each entry); digests are verified on every fetch.",
  };
