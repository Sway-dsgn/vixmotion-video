 ...
 *
 * Security model:
 * - Master password â†’ PBKDF2 (100k iterations, SHA-256) â†’ derived AES-GCM-256 key
...
const SECURE_DB_NAME = "vixmotion-secure";
...
export async function setupMasterPassword(password: string): Promise<void> {
  ...
  const verificationPlaintext = "vixmotion-verify-v1";
...
export async function unlockSession(password: string): Promise<boolean> {
  ...
  try {
    ...
    if (decrypted !== "vixmotion-verify-v1") {
      failedAttempts++;
