import { getBlueprint, validateGraph, type AssetKind, type Graph } from "@vixmotion/fxpkg";
...
export interface TutorialStep {
  ...
  /** "Build this step for me" â€” mutate toward the target stage */
  buildGraph?: () => Graph;
  buildLabel?: string;
}
  ...
  steps: [
    {
      ...
      body:
        "An effect is a graph of nodes. Video flows in from a Source, gets transformed, and leaves through an Output. We'll detect the person, cut them out, make three copies, and place them over a blurred background. Click Next to begin â€” at any step you can press â€œBuild this stepâ€ to do it for you, or build it yourself from the palette.",
    },
    {
      title: "1 Â· Detect the person",
      body:
    ...
    {
      title: "2 Â· Cut them out",
      body:
        "A Cutout node takes the Source and the mask and gives you just the person on transparency. Connect Source â†’ Cutout.source and the mask â†’ Cutout.mask (drag from an output dot to an input dot).",
      check: (g) => hasType(g, "Cutout"),
      buildGraph: () => subgraph(clone(), ["src", "mask", "smooth", "cut"]),
      buildLabel: "Add & wire Cutout",
    },
    {
      title: "3 Â· Make three copies",
      body:
        "Add three Transform nodes, each fed by the Cutout. Offset them left, center, and right (the x value in each node's properties). That's the â€œmultiply a bodyâ€ trick â€” same cutout, three positions.",
      check: (g) => countType(g, "Transform") >= 3,
      buildGraph: () => subgraph(clone(), ["src", "mask", "smooth", "cut", "tl", "tc", "tr"]),
      buildLabel: "Add 3 Transforms",
    },
    {
      title: "4 Â· Blur the background & stack",
      body:
    ...
    {
      title: "Done â€” you built an effect!",
      body:
...
const firstFilter: Tutorial = {
  ...
  steps: [
    ...
    {
      title: "1 Â· Boost the color",
      body: "Add a Saturation node and connect Source â†’ Saturation. Raise its amount in the Inspector to make colors richer.",
      check: (g) => hasType(g, "Saturation"),
      buildGraph: () => subgraph(warm(), ["src", "sat"]),
      buildLabel: "Add Saturation",
    },
    {
      title: "2 Â· Add a vignette",
      body: "A Vignette darkens the edges to draw the eye inward. Connect Saturation â†’ Vignette.",
      check: (g) => hasType(g, "Vignette"),
      buildGraph: () => subgraph(warm(), ["src", "sat", "vig"]),
      buildLabel: "Add Vignette",
    },
    {
      title: "3 Â· Expose a control & finish",
      body:
    ...
    {
      title: "Done â€” it compiles & previews!",
      body:
