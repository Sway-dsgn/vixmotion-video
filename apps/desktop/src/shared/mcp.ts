...
export function endpointFilePath(): string {
  const override = process.env.VIXMOTION_MCP_ENDPOINT_FILE;
  if (override && override.length > 0) return override;
  return path.join(os.homedir(), ".VixMotion", "mcp-endpoint.json");
}

export const MCP_PROTOCOL_PATH = "/mcp";

/**
 * Canonical MCP mainâ†”renderer bridge contract. The desktop side (dispatcher,
 * preload, main) imports these; the web renderer (a separate package that can't
 * import apps/desktop) mirrors them in mcp-listener.ts / global.d.ts â€” keep the
 * two in sync.
 */
