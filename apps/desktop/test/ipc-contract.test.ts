...
describe("ipc-contract", () => {
  it("exposes the probeHardware channel name", () => {
    expect(CHANNELS.probeHardware).toBe("vixmotion:probeHardware");
  });

  it("exposes the rigging backend probe channel name", () => {
    expect(CHANNELS.riggingProbeBackend).toBe("vixmotion:rigging:probeBackend");
  });

  it("exposes the humanoid rigging job channel name", () => {
    expect(CHANNELS.riggingRigHumanoidModel).toBe("vixmotion:rigging:rigHumanoidModel");
  });

  it("exposes the Aurora preview channel name", () => {
    expect(CHANNELS.auroraRenderPreview).toBe("vixmotion:aurora:renderPreview");
  });

  it("exposes the Aurora preview-session channel names", () => {
    expect(CHANNELS.auroraStartPreviewSession).toBe(
      "vixmotion:aurora:startPreviewSession",
    );
    expect(CHANNELS.auroraCancelPreviewSession).toBe(
      "vixmotion:aurora:cancelPreviewSession",
    );
    expect(CHANNELS.auroraPreviewEvent).toBe("vixmotion:aurora:previewEvent");
  });

  it("exposes the Aurora sequence-session channel names", () => {
    expect(CHANNELS.auroraStartSequenceSession).toBe(
      "vixmotion:aurora:startSequenceSession",
    );
    expect(CHANNELS.auroraCancelSequenceSession).toBe(
      "vixmotion:aurora:cancelSequenceSession",
    );
    expect(CHANNELS.auroraSequenceEvent).toBe("vixmotion:aurora:sequenceEvent");
  });
