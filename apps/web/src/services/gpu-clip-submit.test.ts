...
vi.mock("@vixmotion/core", async (importActual) => {
  const actual = await importActual<typeof import("@vixmotion/core")>();
  return {
    ...actual,
...
describe("submitSelectedClipJob", () => {
  it("merges extra params with context and registers the job (web blob path)", async () => {
    ...
    // object_removal is a "replace" kind â†’ no new library tile; the job targets
