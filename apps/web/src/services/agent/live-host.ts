...
} from "@vixmotion/agent";
import type { TextStyle, TextAnimationPreset } from "@vixmotion/core/text/types";
import type { ShapeStyle, ShapeType } from "@vixmotion/core/graphics/types";
import type { Transform } from "@vixmotion/core/types/timeline";
import { CAPABILITY_MANIFEST } from "@vixmotion/core/capabilities/manifest";
import type { Action } from "@vixmotion/core/types/actions";
import type { Project } from "@vixmotion/core/types/project";
import type { CapabilityManifest } from "@vixmotion/core/capabilities/manifest";
...
export class LiveEditorHost implements EditingHost {
  ...
  async exportMotionScene(
    ...
    if (willNormalize && options.acknowledgeH264Fallback !== true) {
      throw new Error(
        "Web export can't produce ProRes or alpha â€” this will encode H.264 without transparency. Pass acknowledgeH264Fallback:true to consent, or use the desktop app for ProRes/alpha.",
      );
    }
  ...
  async createStickerOverlay(
    ...
    const { stickerLibrary } = await import("@vixmotion/core");
