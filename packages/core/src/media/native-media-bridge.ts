...
export function getBridge(): NativeMediaBridge | undefined {
  const w = globalThis as unknown as { VixMotion?: Partial<NativeMediaBridge> };
  const o = w.VixMotion;
...
// Stream a File/Blob to a temp file on disk (chunked â€” bounded peak memory) and return its path.
