...
// Draws `source` cover-fit and Gaussian-blurred across the whole canvas â€” the
...
export const Preview: React.FC = () => {
  ...
  const loadAudioBuffer = async (
    ...
    try {
      const { extractAudioWav } = await import("@vixmotion/core/media");
  ...
  // "blur" backdrop is NOT painted here â€” it needs the actual decoded base
    ...
    const startMultiTrackPlayback = async () => {
      ...
      const processMultiTrackFrame = async () => {
        ...
        try {
          ...
          // (one will be outside its visible window â€” its sink will clamp to
  ...
  useEffect(() => {
    ...
    window.addEventListener("vixmotion:preview-invalidate", handler);
    return () => window.removeEventListener("vixmotion:preview-invalidate", handler);
  }, []);
        ...
        >
          ...
          {!cropMode && showResizeHandles && clipBounds && (
            ...
            >
              ...
              >
                {lockAspectRatio ? "ðŸ”’ Locked" : "ðŸ”“ Free"}
              </Button>
