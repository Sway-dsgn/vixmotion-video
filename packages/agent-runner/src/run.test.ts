...
import { MockLLMClient } from "@vixmotion/agent";
import type { LLMResponse } from "@vixmotion/agent";
...
function tmpFile(): string {
  const file = path.join(tmpdir(), `VixMotion-run-${randomUUID()}.json`);
  tempFiles.push(file);
  return file;
}
afterEach(async () => {
...
describe("runHeadlessEditFile", () => {
  it("loads a stored project, edits it, and saves the result â€” no app", async () => {
