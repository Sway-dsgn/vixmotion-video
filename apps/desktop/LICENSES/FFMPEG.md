# FFmpeg â€” license & attribution

VixMotion Desktop bundles a prebuilt **FFmpeg** binary (one per platform/arch
...
linked into the VixMotion application; it is executed across the OS process
boundary.
...
Public License**. None are built `--enable-nonfree` â€” this is enforced at fetch
...
- The macOS x64, Linux, and Windows builds are additionally configured with
  `--enable-version3` â†’ **GPL-3.0-or-later**.
- The macOS arm64 build is **GPL-2.0-or-later**.

The full license texts are bundled alongside this file: `GPL-3.0.txt` and
`GPL-2.0.txt`.

- FFmpeg project: <https://ffmpeg.org> Â· license overview: <https://ffmpeg.org/legal.html>

FFmpeg is Â© the FFmpeg developers. `libx264` is Â© VideoLAN; `libx265` is
Â© MulticoreWare / the x265 project. All are used under the GPL.
...
| Slot | FFmpeg | Source |
|---|---|---|
| `darwin-arm64` | 7.1 | OSXExperts â€” <https://www.osxexperts.net> (GPL static) |
| `darwin-x64` | 6.1.1 | ffmpeg-static `b6.1.1` (Evermeet build) â€” <https://github.com/eugeneware/ffmpeg-static/releases/tag/b6.1.1> |
...
bundled FFmpeg version â€” together with the build configuration used â€” is
available from the upstream FFmpeg repository at the matching release tag:

- <https://github.com/FFmpeg/FFmpeg> â€” tags `n7.1` (macOS arm64) and `n6.1.1`
  (all other slots).
- Build definitions: <https://github.com/eugeneware/ffmpeg-static> and
  <https://www.osxexperts.net>.

You may also obtain the corresponding source from VixMotion for three years from
the date of distribution by contacting **support@VixMotion.video** *(replace with
...
## Separation from VixMotion

VixMotion Desktop (the Electron application and its own source) is **not** a
