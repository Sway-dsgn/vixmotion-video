    ...
    <div className={embedded ? "" : "flex h-full min-h-0 flex-col"}>
      ...
      <div className={embedded ? "" : "min-h-0 flex-1 overflow-auto"}>
        <Section title="Queue Controls" icon={FileVideo2}>
          <Button
            label={isRunning ? "Renderingâ€¦" : "Start render queue"}
            icon={Play}
            variant="solid"
            disabled={isRunning || runnableCount === 0 || guardrailBlocking}
            onClick={runQueue}
          ...
          <Field label="Output format">
            <SelectInput
              ...
              options={MOTION_EXPORT_FORMATS.map((entry) => ({
                value: entry.id,
                label: `${entry.label}${entry.transparent ? " Â· alpha" : ""}`,
              }))}
              onChange={(format) => {
                if (format === "") return;
        ...
        <Section title={`Jobs Â· ${renderQueue.length}`} icon={ListChecks}>
      ...
      <div className="mb-2 flex items-start justify-between gap-2">
        <span className="min-w-0">
          ...
          <span className="mt-0.5 block text-[10.5px] tabular-nums text-fg-muted">
            {item.width}Ã—{item.height} Â· {item.frameRate} fps Â·{" "}
            {item.duration.toFixed(1)}s Â·{" "}
