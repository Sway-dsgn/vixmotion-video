...
import { getRealtimeAudioGraph } from "@vixmotion/core";
import { ToolcraftIconButton as IconButton } from "@vixmotion/ui";
import { ToolcraftSlider as Slider } from "@vixmotion/ui";
...
export const AudioMixer: React.FC<AudioMixerProps> = ({
  ...
  // Get audio tracks from the timeline (Requirement 20.1) â€“ safe if project/timeline not ready
  ...
  // Handle volume change (Requirement 20.2) â€“ applies to same graph used for playback
  ...
  // Level metering â€“ based on track volume and mute/solo
  useEffect(() => {
    if (!visible) return;
    ...
    >
      ...
      <div className="mt-3 pt-3 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500">
        <span>
          ...
          {hasSoloedTracks && (
            <span className="ml-2 text-yellow-500">â€¢ Solo active</span>
          )}
        </span>
        <span>
