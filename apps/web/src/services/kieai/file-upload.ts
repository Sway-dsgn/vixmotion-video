 ...
 *
 *   uploadFileStream  â€” PRIMARY: browser sends File/Blob bytes directly to
 *                       KieAI via multipart/form-data. Works for any local
 *                       file regardless of size. Use this for media library
 *                       assets (images, videos, audio).
 *
 *   uploadFileBase64  â€” For canvas/thumbnail exports already in base64 form.
 *                       Limited to ~10 MB due to base64 expansion overhead.
 *
 * NOTE: uploadFileByUrl is intentionally not the default here â€” KieAI's
...
/**
 * PRIMARY â€” Upload a local File or Blob as a binary stream.
 *
...
export async function uploadFileStream(
  ...
  form.append("uploadPath", options.uploadPath ?? "VixMotion");
 ...
 * Keep under ~10 MB â€” base64 expands the payload by ~33%.
 *
 ...
 *
 * KieAI's server fetches the file at the given URL â€” localhost URLs will NOT
...
/**
 * Convenience dispatcher â€” picks the right method automatically:
 *
 * - File | Blob              â†’ uploadFileStream  (always preferred for local files)
 * - "data:..." string        â†’ uploadFileBase64
 * - "http..." string         â†’ uploadFileByUrl   (only if publicly reachable)
 */
