...
import { MotionShaderRenderer } from "@vixmotion/core/motion/motion-shader-renderer";
    ...
    <div className="space-y-2" aria-label={label}>
          ...
          return (
            ...
            >
              ...
              <span className="block truncate text-[9px] text-fg-4">
                {def.collection ?? "Built-in"} Â· {def.category}
              </span>
            </button>
          );
        })}
      ...
      {visibleDefs.length === 0 ? (
        <div className="rounded-[8px] border border-dashed border-border px-3 py-5 text-center text-[11px] text-fg-4">
          No {sample === "effect" ? "effects" : "materials"} match â€œ{query}â€.
        </div>
      ) : null}
    </div>
  );
