...
afterEach(() => {
  delete (window as unknown as { VixMotion?: unknown }).VixMotion;
});

describe("shareBaseOrigin", () => {
  it("uses publicOrigin on desktop for share + deep links", () => {
    (window as unknown as { VixMotion: unknown }).VixMotion = {
      platform: "desktop",
      publicOrigin: "https://app.VixMotion.video",
    };
    expect(shareBaseOrigin()).toBe("https://app.VixMotion.video");
    expect(generateShareableLink("share")).toMatch(/^https:\/\/app\.VixMotion\.video#\//);
    expect(getSharePageUrl("x")).toBe("https://app.VixMotion.video#/share/x");
  });
