...
import { executeTool, type ToolResult } from "@vixmotion/agent";
import {
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 text-[10.5px] tabular-nums text-fg-muted">
              {scene.objectCount} obj Â· {scene.cameraCount} cam Â· {scene.animationCount} anim
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <PanelCopy className="mt-1 truncate text-[10.5px] text-fg-muted">
                    {layer.compositionName} Â· {layer.objectCount} render object(s)
                  </PanelCopy>
                </div>
                <Button
      ...
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          ...
          <PanelCopy className="mt-0.5 text-[10px] tabular-nums text-fg-muted">
            {formatSeconds(cueTime)}-{formatSeconds(clip.lastTime ?? clip.duration)} Â· {clip.trackCount} tracks Â· {clip.keyframeCount} keys
          </PanelCopy>
        </div>
        <IconButton
          label={`Cue ${clip.name}`}
          ...
          >
            <div className="min-w-0">
              ...
              <PanelCopy className="mt-0.5 truncate tabular-nums text-fg-muted">
                {track.channel} Â· {track.keyframeCount} keys Â· {formatTrackRange(track.firstTime, track.lastTime)}
              </PanelCopy>
            </div>
            <span
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 truncate text-[10px] text-fg-muted">
          ...
          {object.materialId ? ` Â· ${object.materialId}` : " Â· no material"}
        </div>
      </div>
