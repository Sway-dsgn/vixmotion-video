...
import { ToolcraftSegmentedControl } from "@vixmotion/ui";
...
import { ToolcraftFileDropControl as FileInput } from "@vixmotion/ui";
import { ToolcraftNumberInputControl } from "@vixmotion/ui";
import { ToolcraftProgressBar as ProgressBar } from "@vixmotion/ui";
...
const QUALITY_TIERS: {
  ...
  { value: "balanced", label: "Balanced", hint: "â‰ˆ720p" },
...
export function CompressDialog({ isOpen, onClose }: CompressDialogProps) {
  ...
  const handlePick = useCallback(async (picked: File) => {
    ...
    try {
      ...
      if (!probed) {
        setError("Couldn't read that video â€” try a different file.");
      }
      setSource(probed);
    } catch {
      setError("Couldn't read that video â€” try a different file.");
  ...
  const handleCompress = useCallback(async () => {
    ...
    try {
      ...
      if ((err as Error)?.name !== "AbortError") {
        setError("Compression failed â€” try a lighter setting.");
      }
    } finally {
          ...
          {compressing && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                Compressingâ€¦ {Math.round(progress * 100)}%
                </Text>
              </div>
              <ProgressBar
