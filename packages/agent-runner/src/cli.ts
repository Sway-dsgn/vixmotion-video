...
const USAGE = `VixMotion-agent â€” headless project editing

Usage:
  VixMotion-agent --project <file.json> --prompt "<instruction>" [options]
...
API key (never stored): VixMotion_API_KEY, or ANTHROPIC_API_KEY / OPENAI_API_KEY.`;
...
async function main(): Promise<void> {
  ...
  if (!apiKey) {
    process.stderr.write(
      `No API key found. Set VixMotion_API_KEY (or ${args.provider === "anthropic" ? "ANTHROPIC_API_KEY" : "OPENAI_API_KEY"}).\n`,
    );
    process.exit(1);
  }
