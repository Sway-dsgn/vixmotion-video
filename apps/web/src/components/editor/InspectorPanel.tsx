...
import type { Transform, EditingTemplatePrimitive } from "@vixmotion/core";
import {
  ChromaKeyEngine,
...
import { VixMotion_TRANSCRIBE_URL } from "../../config/api-endpoints";
  ...
  const applyClipEffectWithPlaybackLock = useCallback(
    async (
      ...
      try {
        ...
        await apply();
        window.dispatchEvent(new CustomEvent("vixmotion:preview-invalidate"));
        await waitForEffectApplicationPaint();
      } finally {
  ...
  const handleGenerateSubtitles = useCallback(async () => {
    ...
    try {
      const transcriptionService = initializeTranscriptionService({
        apiEndpoint: `${VixMotion_TRANSCRIBE_URL}/transcribe`,
