...
const VERSION_MARKER_FILE = ".VixMotion-build";
...
function readMarker(markerPath: string): string {
  try {
    ...
    // Corrupt marker (e.g. a directory) â€” remove it so a good one can be written.
    rmSync(markerPath, { recursive: true, force: true });
  } catch {
    // Missing or unreadable â†’ treat as first run.
  }
  return "";
}
...
export function migrateGpuCacheOnUpgrade(): void {
  try {
    ...
    // launch retries â€” otherwise a stale cache that crashes the GPU process
