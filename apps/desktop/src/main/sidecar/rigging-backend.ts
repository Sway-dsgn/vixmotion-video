...
export async function probeRiggingBackend(): Promise<RiggingBackendProbe> {
  ...
  return {
    available: false,
    provider: "blender",
    error: `Blender not found. Checked ${attempted.length} candidate(s). Set VixMotion_BLENDER_PATH or bundle Blender under resources/rigging/blender/<platform>-<arch>.`,
  };
}
...
async function tempRiggingDir(): Promise<string> {
  return mkdtemp(path.join(os.tmpdir(), "vixmotion-rigging-"));
}
...
def parse_args():
    ...
    parser.add_argument("--name", default="VixMotionHumanoid")
    ...
    for obj in meshes:
        ...
        try:
            ...
            if not any(mod.type == "ARMATURE" and mod.object == armature_obj for mod in obj.modifiers):
                mod = obj.modifiers.new("VixMotion Armature", "ARMATURE")
