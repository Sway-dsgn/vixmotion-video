...
export function buildMenuTemplate(platform: string): MenuNode[] {
  ...
  if (isMac) {
    template.push({
      label: "VixMotion",
      submenu: [
        { label: "About VixMotion", role: "about" },
        { label: "Settingsâ€¦", accelerator: "Cmd+,", actionId: "settings" },
  ...
  template.push({
    ...
    submenu: [
      ...
      { label: "Openâ€¦", accelerator: "CmdOrCtrl+O", actionId: "open" },
      { label: "Exportâ€¦", accelerator: "CmdOrCtrl+E", actionId: "export" },
    ],
  });
  template.push({
    label: "Edit",
    submenu: [
      ...
      ...(!isMac
        ? [{ label: "Settingsâ€¦", accelerator: "Ctrl+,", actionId: "settings" }]
        : []),
    ],
  });
  template.push({
...
export function sendMenuAction(win: BrowserWindow | null, id: string): void {
  if (win) win.webContents.send("vixmotion:menu:action", id);
}
