/*
 * VixMotion Creation Core - C ABI boundary.
 *
 * This header defines the stable C ABI that the native (C++20) creation engine
 * exposes to the TypeScript/Electron host via @vixmotion/creation-bindings.
 *
 * The deterministic CPU reference implementation of every function here lives in
 * the TypeScript package @vixmotion/core/creation (geometry/sdf/rig/sim/render).
 * The native build must produce bit-compatible results for the golden tests.
 */
#ifndef VIXMOTION_CREATION_CORE_H
#define VIXMOTION_CREATION_CORE_H
 ...
 * Reference: buildBox() + computeMeshStats() in @vixmotion/core/creation/geometry.
 */
...
#endif /* VIXMOTION_CREATION_CORE_H */
