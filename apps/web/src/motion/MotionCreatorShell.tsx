...
import type { MotionComposition } from "@vixmotion/core";
import { ToolcraftClickableCard, ToolcraftText } from "@vixmotion/ui";
...
const LEFT_PANEL_STORAGE_KEY = "VixMotion.motionCreator.leftPanelWidth.v2";
const RIGHT_PANEL_STORAGE_KEY = "VixMotion.motionCreator.rightPanelWidth.v2";
const TIMELINE_HEIGHT_STORAGE_KEY = "VixMotion.motionCreator.timelineHeight.v2";
...
export function MotionCreatorShell({
  ...
  useEffect(() => {
    ...
    window.addEventListener("vixmotion:menu:export", openMotionExport);
    return () => {
      window.removeEventListener("vixmotion:menu:export", openMotionExport);
    };
  }, [exportCurrentScene]);
    ...
    >
      ...
      <footer className="flex h-7 shrink-0 items-center justify-between border-t border-border bg-bg-1 px-2.5 text-[11px] text-fg-muted">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-fg-3">{composition.width}Ã—{composition.height}</span>
      ...
      {open ? (
        <>
          ...
          <div className="absolute left-1/2 top-[calc(100%+6px)] z-50 w-72 -translate-x-1/2 overflow-hidden rounded-lg border border-border bg-bg-elev shadow-lg">
            ...
            >
              Sequences Â· {compositions.length}
            </ToolcraftText>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <ToolcraftText type="supporting" color="secondary" className="tabular-nums">
                        {scene.width}Ã—{scene.height} Â· {scene.duration.toFixed(1)}s
                      </ToolcraftText>
                    </span>
                    {active ? (
