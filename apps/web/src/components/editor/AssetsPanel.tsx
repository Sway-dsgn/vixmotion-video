...
import type { ShapeType, TextStyle } from "@vixmotion/core";
import { useProjectStore } from "../../stores/project-store";
import { useUIStore } from "../../stores/ui-store";
import { useTimelineStore } from "../../stores/timeline-store";
import type { MediaItem } from "@vixmotion/core";
...
import { ToolcraftIconButton as IconButton } from "@vixmotion/ui";
import { ToolcraftSelectableCard as SelectableCard } from "@vixmotion/ui";
  ...
  const hoverOverlay = (
    <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center gap-2 animate-in fade-in duration-200">
      {item.kieaiError ? (
        ...
        <div title="KieAI generation in progressâ€¦" className="p-2">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-[9px] text-fg-muted">
            ...
            {item.metadata?.duration && formatResolution() && <span>â€¢</span>}
            {formatResolution() && <span>{formatResolution()}</span>}
            {(item.metadata?.duration || formatResolution()) && formatFileSize(item.metadata?.fileSize) && <span>â€¢</span>}
        ...
        {isHovered && (
          <div className="flex items-center gap-1 flex-shrink-0">
            {item.kieaiError ? (
              ...
              <div className="p-1" title="Generatingâ€¦">
...
export const AssetsPanel: React.FC = () => {
  ...
  // Handle drag and drop import â€” capture FileSystemFileHandle for each dropped file
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      e.preventDefault();
      setIsDragOver(false);

      // Snapshot dataTransfer synchronously â€” it becomes inert after the first await.
          ...
          ? Array.from(e.dataTransfer.items)
              ...
              .map(async (item) => {
                try {
                  ...
                  // Ignore â€” handle capture is best-effort
                }
              })
          : [];
  ...
  const handleRelinkFromFolder = useCallback(async () => {
    ...
    // Build a name:size â†’ {File, handle} map for reliable matching
    ...
    for (const item of placeholders) {
      ...
      if (entry) {
        setImportProgress(`Relinking ${item.name}â€¦`);
        try {
            ...
            <div className="min-h-0 flex-1 overflow-auto">
              <div className="px-4 py-4">
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      { type: "mesh-cube" as ShapeType, label: "Cube", icon: "â–¡" },
                      { type: "mesh-sphere" as ShapeType, label: "Sphere", icon: "â—‹" },
                      { type: "mesh-torus" as ShapeType, label: "Torus", icon: "â—¯" },
                      { type: "mesh-cone" as ShapeType, label: "Cone", icon: "â–³" },
                      { type: "mesh-cylinder" as ShapeType, label: "Cylinder", icon: "â–¯" },
                      { type: "mesh-icosahedron" as ShapeType, label: "Icosahedron", icon: "â—†" },
                    ]).map((mesh) => (
                      <PanelButton
                        key={mesh.type}
                        label={mesh.label}
