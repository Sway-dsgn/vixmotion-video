/**
 * Studio API client (STUDIO_PLAN Â§34.2). Talks to the marketplace/studio Worker.
 ...
 */
import type { AssetKind, Graph } from "@vixmotion/fxpkg";
...
async function call<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok && res.status !== 422) {
    throw new Error(`${init?.method ?? "GET"} ${path} â†’ ${res.status}`);
  }
