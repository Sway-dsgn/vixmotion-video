# Phase 2 Acceptance â€” Native Export Parity (Task 2.11)

Native desktop export must beat the browser/wasm path on the two things that path
could not do: real ProRes (it silently downgraded ProRes â†’ H.264) and encodes above
...
Run with: `pnpm --filter @VixMotion/desktop test:run export-integration`
...
backpressure handling (no in-RAM video buffering â€” frames go straight to ffmpeg's
stdin and ffmpeg writes the file to disk).

1. **Native H.264, frames + audio â†’ mp4** â€” hardware-selected encoder
...
2. **Native ProRes â†’ `.mov` (Case A)** â€” encoder `prores_videotoolbox`
...
3. **Higher-than-1080p encode (Case B)** â€” `h264_videotoolbox` at **2560x1440**, a
...
4. **Streamed-to-disk via stdin** â€” every case writes frames through
...
2. **Native save dialog + direct-to-disk write.** Confirm the native OS save dialog
   appears, and the chosen file is written **directly to the selected path** â€” not
...
Tracked as a v1 limitation â€” see spec Â§13.

# Desktop GPU AI panel â€” pending human E2E (Task D5)

The GPU cloud-jobs pipeline (token mint â†’ presign â†’ upload â†’ poll â†’ artifact import)
...
2. **Confirm the broker + GPU base URLs point at live services.** Verify
   `VixMotion_AUTH_BROKER_BASE_URL`
   (default `https://VixMotion-cloud.niiyeboah1996.workers.dev`) and
   `VixMotion_GPU_BASE_URL` (default `https://ai.VixMotion.video`) resolve to the
   deployed Worker and the live GPU render service.

3. **Manual E2E â€” upscale a clip end to end.** Launch the desktop app, open the AI
   panel via the **Sparkles** toolbar toggle, select a single clip, and run
   `upscale`. Confirm the sequence: token mint â†’ presign â†’ upload of the source temp
   file â†’ status poll until `completed` â†’ artifact download â†’ import back into the
   ...
   **in place** (same media id, not a duplicate), is no longer stuck pending, and has
   ...
   duration â€” not mislabeled as an image).
...
# Desktop Native Pro Redesign â€” pending human checks (Phases 1â€“2)
...
- [ ] The DaVinci-style charcoal/teal theme renders on the shell (title bar, workspace tabs, page bodies) â€” surfaces charcoal, accent teal, shadcn primitives (Select/DropdownMenu/focus rings) teal not emerald.
- [ ] Edit/Color/Deliver page tabs switch and persist across reload (ui-store desktopPage).
- [ ] Packaged build (`pnpm --filter @VixMotion/desktop run dist` with certs) â€” measure real installer size + cold-start per platform. Signed/notarized dist requires certs (mac hardened-runtime + notarize creds; Windows Authenticode) supplied via CI/env; per-platform native ffmpeg binaries must be present in resources/bin.
...
# Desktop polish pass â€” pending human checks (PP1â€“PP7)
...
- [ ] VixMotion app icon shows in the macOS dock (packaged build) and in the Windows taskbar / window chrome (`BrowserWindow.icon` on win/linux).
