...
const EffectItem: React.FC<{
  ...
  return (
    <div
      ...
      onDragOver={(event) => {
        if (event.dataTransfer.types.includes("application/x-VixMotion-effect-order")) {
      ...
      onDrop={(event) => {
        const sourceEffectId = event.dataTransfer.getData(
          "application/x-VixMotion-effect-order",
        );
  ...
  return (
    ...
    >
      ...
      <span className="block truncate text-[9px] text-fg-4">
        {def.category} Â· {def.description}
      </span>
    </button>
  );
}
      ...
      content={
        <div className="w-[430px] max-w-[calc(100vw-32px)] space-y-2.5 p-2.5">
          <div className="flex items-center gap-1 rounded-[8px] bg-bg-2 p-1">
            {([
              ["effects", `Effects Â· ${EDITOR_EFFECT_PREVIEWS.length}`],
              ["shaders", `Shaders Â· ${shaderDefs.length}`],
