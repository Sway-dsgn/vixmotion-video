# VixMotion Desktop â€” Distribution Guide
...
**Signing identity** â€” configured in `electron-builder.yml` as
...
**Notarization** â€” `mac.notarize.teamId: 864H636QW4` is set; `notarytool` is
installed (Xcode). Provide credentials ONE of two ways:

```bash
# Option A: saved keychain profile (simplest locally) â€” run once:
xcrun notarytool store-credentials VixMotion-notary \
  --apple-id "you@example.com" --team-id 864H636QW4 \
  --password "abcd-efgh-ijkl-mnop"        # app-specific password (appleid.apple.com)
export APPLE_KEYCHAIN_PROFILE=VixMotion-notary

# Option B: env (CI) â€” Apple ID:
export APPLE_ID="you@example.com"
export APPLE_APP_SPECIFIC_PASSWORD="abcd-efgh-ijkl-mnop"
export APPLE_TEAM_ID="864H636QW4"

# Option B': env (CI) â€” App Store Connect API key (preferred for CI):
...
In the release CI these come from the `WIN_CSC_LINK` / `WIN_CSC_KEY_PASSWORD` secrets. (Without them, the Windows installer is unsigned and triggers SmartScreen warnings â€” acceptable for internal testing only; the release workflow still builds it.)

---

## 5. Hosting + auto-update (R2 â†’ dl.VixMotion.video)

Built apps are **not** on GitHub Releases. They live in a Cloudflare **R2**
bucket served at **`https://dl.VixMotion.video`**, which hosts:

- the installers â€” `VixMotion-<version>-arm64.dmg`, `-x64.dmg`, `-x64.exe`, `-x86_64.AppImage`, `-amd64.deb` (electron-builder's arch token differs per target: x64 â†’ `x86_64` for AppImage, `amd64` for deb),
- the electron-updater feed (`latest.yml`, `latest-mac.yml`, `latest-linux.yml`, `*.zip`, `*.blockmap`),
- `manifest.json` â€” the human-download index the landing page reads.

`electron-builder.yml` sets `publish: { provider: generic, url: https://dl.VixMotion.video }`, so each build embeds that update feed and emits `latest*.yml` + blockmaps (the generic provider does not upload â€” the CI does, Â§6).

**Self-update flow** (`src/main/updater.ts`, `initAutoUpdater`): on launch it checks the R2 feed in the background (`autoDownload = false`). When a newer version exists the renderer shows an **update banner** (`apps/web/src/desktop/UpdateBanner.tsx`); the user clicks **Download** (`window.VixMotion.updater.download()` â†’ in-app progress) and then **Restart & Install** (`updater.install()`), which quits through the normal flow so `autoInstallOnAppQuit` applies the update **after** the unsaved-changes guard. Nothing downloads or installs without consent.

Caveats: macOS install requires the build to be **signed** (Squirrel.Mac) â€” the CI does not upload an unsigned mac update feed; Linux auto-update is **AppImage** only (not `.deb`). The DMG/installers are always uploaded for manual download regardless of signing.

### R2 setup (one-time, in the Cloudflare dashboard)

1. **Create a bucket** (e.g. `VixMotion-desktop`).
2. **Attach the custom domain** `dl.VixMotion.video` to the bucket (R2 â†’ bucket â†’ Settings â†’ Public access â†’ Custom Domains). This makes objects public at `https://dl.VixMotion.video/<key>`.
3. **Create an R2 API token** (Account â†’ R2 â†’ Manage API Tokens) with Object Read & Write on that bucket; note the Access Key ID + Secret + your Account ID.
4. **CORS** so the landing (a different origin) can `fetch` `manifest.json` â€” already configured on the bucket via `wrangler r2 bucket cors set` for `https://VixMotion.video`, `www.`, `app.`, and `localhost:5173/3000` (dev). The desktop updater runs from the main process and is not CORS-bound. Add any new landing origin (e.g. a `*.pages.dev` preview) to that policy.

---

## 6. Release CI (build â†’ R2)

`.github/workflows/release.yml` runs a `quality` gate (typecheck + tests) then
builds all three platforms on a matrix (macos â†’ dmg/zip arm64+x64, windows â†’
nsis x64, ubuntu â†’ AppImage/deb x64) on a `v*` tag (or `workflow_dispatch` with
...
| `R2_BUCKET` | the bucket name (e.g. `VixMotion-desktop`) |
...
writes `manifest.json` â€” clients then see the update and the landing page shows
the new downloads. No manual publish step. (`desktop-readiness.yml` is an
unrelated stale Rust workflow; remove separately.)

### Landing page

`../VixMotion-landing` renders a desktop download section
(`components/DesktopDownload.tsx`) that fetches `https://dl.VixMotion.video/manifest.json`
at runtime and shows per-OS buttons (OS-detected primary CTA + all-platforms
list). No rebuild of the landing is needed for a new desktop version â€” it reads
the live manifest.
...
- Product name: **VixMotion** Â· appId: `video.VixMotion.desktop` Â· version: `apps/desktop/package.json`.
- Note: the GPU bundle id referenced in `src/main/index.ts` (`com.VixMotion.video`) differs from `video.VixMotion.desktop` â€” reconcile if a single identity is desired.

## What we (the product owner) must provide
1. ~~ffmpeg binaries / GPL attribution / auto-update~~ â€” **done** (Â§2, Â§5).
2. **R2 + subdomain** (one-time): create the bucket, bind `dl.VixMotion.video` to it, mint an R2 API token (Â§5), and set `R2_ACCOUNT_ID` / `R2_BUCKET` / `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` as CI secrets.
3. **macOS signing**: `MAC_CSC_LINK` / `MAC_CSC_KEY_PASSWORD` + `APPLE_ID` / `APPLE_APP_SPECIFIC_PASSWORD` / `APPLE_TEAM_ID` (Â§3, Â§6) â€” required for mac auto-update to install.
4. **Windows signing**: `WIN_CSC_LINK` / `WIN_CSC_KEY_PASSWORD`, or accept unsigned Windows (Â§4).
