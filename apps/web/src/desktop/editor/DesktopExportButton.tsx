...
import type { VideoExportSettings } from "@vixmotion/core";
import { setEncoderBackendFactory, WebCodecsBackend } from "@vixmotion/core";
...
const resolveExportOutputPath = (): string =>
  (window as { __VixMotionExportPath?: string }).__VixMotionExportPath ?? "";

// Native ffmpeg writes the file itself via the resolved output path, so it does
// not consume the writable stream â€” a no-op stream satisfies the interface.
const NOOP_WRITABLE = {
  async seek() {},
  async write() {},
  async close() {},
...
export function DesktopExportButton(): JSX.Element {
  ...
  useEffect(() => {
    ...
    window.addEventListener("vixmotion:menu:export", open);
    return () => window.removeEventListener("vixmotion:menu:export", open);
  }, []);
          ...
          setEncoderBackendFactory((mediabunny) =>
            mediabunny
              ? new WebCodecsBackend(mediabunny, {
                  ...
                  // so export at the full (matched) resolution â€” no browser clamp.
