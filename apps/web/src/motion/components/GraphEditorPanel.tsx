...
import { ToolcraftText } from "@vixmotion/ui";
...
const EASING_OPTIONS = EASING_CATEGORIES.flatMap((category) =>
  category.easings.map((easing) => ({
    value: easing,
    label: `${category.name} Â· ${formatEasingName(easing)}`,
  })),
);
...
function KeyframeGraphPoint({
  ...
  return (
    ...
    >
      <title>
        {point.roving
          ? `${point.time.toFixed(2)}s Â· roving`
  ...
  if (isFlatSegment(segment.from.value, segment.to.value)) {
    ...
    return (
      <g data-testid={`flat-segment-hint-${segment.from.id}`}>
        <title>Flat segment â€” handles have no effect</title>
        <circle
          cx={midX}
          cy={midY}
          r={3}
