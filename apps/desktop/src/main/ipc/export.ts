...
export async function startExport(wc: WebContents, args: ExportStartArgs): Promise<{ jobId: string }> {
  ...
  const audioWavPath = path.join(os.tmpdir(), `VixMotion-${jobId}.wav`);
  ...
  // per-job queue keeps exactly one push (and one ffmpeg stdin write) in flight â€”
  ...
  wc.postMessage("vixmotion:export-port", { jobId }, [port2]);
