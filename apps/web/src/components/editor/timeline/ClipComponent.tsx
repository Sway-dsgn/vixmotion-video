...
import { ToolcraftContextMenu as ContextMenu } from "@vixmotion/ui";
import { Box, Image, Layers } from "@/icons/lucide-compat";
import type { Clip, Track, TransitionType } from "@vixmotion/core";
...
export const ClipComponent: React.FC<ClipComponentProps> = ({
  ...
  // â”€â”€ Drag-drop: effects & transitions from the assets panel â”€â”€â”€â”€
  ...
  // we treat the outer ~25% of the clip's width as an "edge zone" â€” the
  // closer edge wins, and we map left edge â†’ incoming, right edge â†’
  // outgoing transition.
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      if (kind === null) {
        // Don't preventDefault â€” let other handlers (e.g. timeline file
        // drop) take over.
        return;
      }
      e.preventDefault();
  ...
  const handleDragLeave = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    // Only clear when the pointer actually exits the clip â€” dragleave
          ...
          toast.success(
            transition.edge === "in"
              ...
              : transition.edge === "out"
                ...
                : "Transition applied",
            `${transitionType} â€¢ ${transition.duration.toFixed(1)}s`,
          );
          return;
        }
      }
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!effectType && !transitionType) {
        // Not for us â€” let the timeline's outer drop handler take it.
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      pendingCommitRef.current = () => {
        ...
        // companions along â€” that gets too lossy when they live on tracks
        // of a different type â€” but same-track drags stay locked.
