...
import { getMotionLayerPropertyValueAtTime } from "@vixmotion/core/motion/motion-keyframes";
import type { MotionLayer } from "@vixmotion/core/motion/types";
...
describe("executeTool", () => {
  ...
  it("adds semantic decals and recovers render bindings", async () => {
    ...
    expect(renderObject).toMatchObject({
      object: { kind: "text3d", text: "VixMotion", extrude: 0.015 },
      material: {
        color: "#f8fafc",
        emissive: "#f8fafc",
        emissiveIntensity: 0.75,
    ...
    expect(recoveredDecal).toMatchObject({
      object: { kind: "text3d", text: "VixMotion" },
      material: { color: "#f8fafc", emissive: "#f8fafc" },
      opacity: 0.92,
    });
    const validation = (await executeTool("validate_creation_state", {}, host)).data as {
