...
import {
  ...
  type RayTracedImage,
} from "@vixmotion/core/creation/index";
...
function nativeAddonCandidates(): readonly string[] {
  ...
  const explicit = process.env.VixMotion_CREATION_ADDON_PATH?.trim();
...
export function loadNativeAddon(
  ...
  const require = createRequire(path.resolve(process.cwd(), "__VixMotion_creation_bindings__.cjs"));
...
interface NativeHostGlobal {
  __VixMotionCreationNative?: NativeCreationModule;
  __VixMotionCreationWasm?: NativeCreationModule;
}
...
export function loadCreationBackend(
  ...
  const native = options.nativeModule ?? host.__VixMotionCreationNative;
  if (native) return wrapNative(native, "native");
  const wasm = options.wasmModule ?? host.__VixMotionCreationWasm;
