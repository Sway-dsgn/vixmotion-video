...
def test_build_manifest_entry_includes_sha_and_bytes(tmp_path: Path):
    ...
    entry = build_manifest_entry(
        recipe=recipe,
        cube_path=cube_path,
        base_url="https://filters.VixMotion.video",
    )
    assert entry["id"] == "cinematic.demo"
    assert entry["cubeUrl"] == "https://filters.VixMotion.video/cube/cinematic.demo.cube"
        ...
        filters=[
            {
                ...
                "accent": "#000000",
                "sort": 1,
                "cubeUrl": "https://filters.VixMotion.video/cube/x.y.cube",
                "sha256": "a" * 64,
                "bytes": 100,
            }
        ],
