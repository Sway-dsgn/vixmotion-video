...
//   electron-builder's artifactName: VixMotion-<version>-<arch>.<ext>.

import { readdirSync, statSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const BASE_URL = "https://dl.VixMotion.video";
...
const files = walk(artifactsDir).filter((f) =>
  path.basename(f).startsWith(`VixMotion-${version}-`),
);
...
// x64 â†’ "x64" for dmg/exe but "x86_64" for AppImage and "amd64" for deb.
const targets = {
