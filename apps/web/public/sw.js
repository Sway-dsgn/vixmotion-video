...
async function getCacheStatus() {
  ...
  return {
    cacheNames: cacheNames.filter((n) => n.startsWith("vixmotion-")),
    totalEntries,
    version: CACHE_NAME,
  };
}
...
/**
 * Clear all VixMotion caches
 */
async function clearAllCaches() {
  const cacheNames = await caches.keys();
  await Promise.all(
    cacheNames
      .filter((name) => name.startsWith("vixmotion-"))
