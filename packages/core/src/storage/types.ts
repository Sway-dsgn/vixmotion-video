...
export const DB_NAME = "VixMotion-db";
...
/** Keyed by "${name}:${size}" â€” allows restoring assets by filename+size across sessions */
export interface FileHandleRecord {
  readonly key: string; // "${name}:${size}"
  readonly handle: FileSystemFileHandle;
}

/** Keyed by projectId â€” stores the last folder the user relinked from, per project */
