import type { AiCloudJobCreated, AiCloudJobStatusResponse } from "@vixmotion/core";

const BUNDLE_ID = "com.VixMotion.video";
const INSTANCE_ID_STORAGE_KEY = "VixMotion.gpu.instanceId";
...
function brokerBaseUrl(): string {
  const fromEnv = import.meta.env?.VITE_VIXMOTION_AUTH_BROKER_BASE_URL as string | undefined;
  return trimTrailingSlash(fromEnv && fromEnv.length > 0 ? fromEnv : "https://api.VixMotion.video");
}

function gpuBaseUrl(): string {
  const fromEnv = import.meta.env?.VITE_VIXMOTION_GPU_BASE_URL as string | undefined;
  return trimTrailingSlash(fromEnv && fromEnv.length > 0 ? fromEnv : "https://ai.VixMotion.video");
}
